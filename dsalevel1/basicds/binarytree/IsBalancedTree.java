...
public class IsBalancedTree {
	...
	public static BalPair isBalanced(Node root) {
		...
		if (lp.isbal == true && rp.isbal == true && (Math.abs(lp.height - rp.height) <= 1)) {

			int nh = Math.max(lp.height, rp.height) + 1;
			BalPair p = new BalPair(true, nh);
			return p;

		} else {
			BalPair p = new BalPair(false, -1);
			return p;
		}
	}
