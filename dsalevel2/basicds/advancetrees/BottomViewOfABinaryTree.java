...
public class BottomViewOfABinaryTree {
	...
	public static ArrayList<Integer> BottomView(TreeNode root) {
		...
		while(que.size() > 0) {
			...
			//this is different from top view 
			//As we have to change array till we get value for that level
			ans.set(index, n.val);
			
			if (n.left != null) {
				que.add(new Pair(n.left, level-1));
			}
			
			if (n.right != null) {
				que.add(new Pair(n.right, level+1));
			}
